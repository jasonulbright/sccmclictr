...
namespace sccmclictr.automation
{
    ...
    public class SCCMAgent : IDisposable
    {
        ...
        internal int connectIPC(string hostname, PSCredential credential)
        {
            ...
            resource.RemoteName = @"\\" + hostname;
            IntPtr password = IntPtr.Zero;
            try
            {
                password = Marshal.SecureStringToGlobalAllocUnicode(credential.Password);
                return WNetAddConnection3(IntPtr.Zero, ref resource, password, credential.UserName, 0);
            }
            finally
            {
        ...
        public bool ConnectIPC_
        {
            ...
            set { if (value) ConnectIPC(Credential); }
        }
        ...
        public bool ConnectIPC(PSCredential credential)
        {
            ...
            try
            {
                int result = credential == null ? 0 : connectIPC(Hostname, credential);
                ipcconnected = result == 0;
                if (!ipcconnected)
                    PSCode.TraceEvent(TraceEventType.Error, result, "IPC connection failed: " + result);
                return ipcconnected;
            }
            catch (Exception ex)
            {
                ipcconnected = false;
                PSCode.TraceEvent(TraceEventType.Error, 0, ex.Message);
                return false;
            }
        }

        public bool ConnectIPC(string username, System.Security.SecureString password)
        {
            return ConnectIPC(new PSCredential(username, password));
        }
        ...
        private static extern int WNetAddConnection3(IntPtr hWndOwner,
            ref NETRESOURCE lpNetResource, IntPtr lpPassword,
